...
void ContentSelectorView::ContentSelector::setProfileContent(const QStringList &fileList)
{
    ...
    const ContentSelectorModel::EsmFile* selectedGameFile = nullptr;
    for (const QString &filepath : fileList)
    {
        ...
        if (file && file->isGameFile())
        {
            ...
            if (file->fileName().compare(
                    QLatin1String("Morrowind.esm"), Qt::CaseInsensitive) == 0)
            {
                selectedGameFile = file;
                break;
            }
    ...
    // A newly created or legacy launcher profile may not contain an absolute
    // master path yet. Prefer Morrowind.esm automatically so the add-on list is
    // enabled and visible immediately after the Wizard finishes.
    if (!selectedGameFile)
    {
        const QStringList availableGameFiles = mContentModel->gameFiles();
        for (const QString& gameFileName : availableGameFiles)
        {
            const ContentSelectorModel::EsmFile* file = mContentModel->item(gameFileName);
            if (!selectedGameFile)
                selectedGameFile = file;
            if (gameFileName.compare(
                    QLatin1String("Morrowind.esm"), Qt::CaseInsensitive) == 0)
            {
                selectedGameFile = file;
                break;
    ...
    if (selectedGameFile)
        setGameFile(selectedGameFile->filePath());
