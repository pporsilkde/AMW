...
const ContentSelectorModel::EsmFile *ContentSelectorModel::ContentModel::item(const QString &name) const
{
    ...
    if (name.contains(QLatin1Char('/')) || name.contains(QLatin1Char('\\')))
    {
        ...
        lookupName = QDir::fromNativeSeparators(name);
    }

    for (const EsmFile *file : mFiles)
    {
        QString candidate = file->fileProperty(fp).toString();
        if (fp == EsmFile::FileProperty_FilePath)
            candidate = QDir::fromNativeSeparators(candidate);

        if (lookupName.compare(candidate, Qt::CaseInsensitive) == 0)
            return file;
    }
    return nullptr;
}
