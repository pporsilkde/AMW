    ...
    , mSilent(silent)
{
    ...
    // Keep configuration, saves, logs and screenshots with this client build.
    // Fall back only when the installation directory is genuinely read-only.
    if (!ensureDirectory(mUserConfigPath))
    {
    ...
    mCachePath = mUserDataPath / "cache";
    ensureDirectory(mCachePath);
    setupTokensMapping();
