...
void ESM::CellRef::loadId (ESMReader& esm, bool wideRefNum)
{
    ...
    mRefID = esm.getHNOCompatRefId ("NAME");
    if (mRefID.empty() && esm.getFormat() <= 15)
    {
    ...
    while (!isLoaded && esm.hasMoreSubs())
    {
        ...
        switch (esm.retSubName().intval)
        {
            ...
            case ESM::FourCC<'K','N','A','M'>::value:
                mKey = esm.getCompatRefId();
                break;
            case ESM::FourCC<'T','N','A','M'>::value:
                mTrap = esm.getCompatRefId();
                break;
