...
void ESM::NpcStats::load (ESMReader &esm)
{
    ...
    // OpenMW 0.50+ stores a temporary crime-related disposition modifier
    // before the skill array. ArenaMW 0.47 has no matching runtime field, but
    // the value still has to be consumed or the first DISM subrecord is
    // mistaken for the first STBA skill value and the whole CSTA record loses
    // synchronization.
    int crimeDispositionModifier = 0;
    esm.getHNOT (crimeDispositionModifier, "DISM");
    ...
    while (esm.isNextSub ("USED"))
        mUsedIds.push_back (esm.getCompatRefId());
