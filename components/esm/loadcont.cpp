...
namespace ESM
{
    ...
    void InventoryList::add(ESMReader &esm)
    {
        ...
        ContItem ci;
        esm.getT(ci.mCount);
        const uint32_t remaining = esm.getSubSize() >= 4 ? esm.getSubSize() - 4 : 0;
        if (esm.getFormat() > 23)
            ci.mItem = esm.getCompatRefIdData(remaining);
        else
            ci.mItem.assign(esm.getString(static_cast<int>(remaining)));
        mList.push_back(ci);
    }
