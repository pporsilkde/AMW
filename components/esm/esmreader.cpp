...
std::string CompatRefId::toLegacyString() const
{
    if (isString())
        return mString;
    ...
    {
        case Generated:
            stream << "sys::compat::generated::" << std::setw(16) << mGenerated;
            return stream.str();

        case FormId:
            stream << "sys::compat::form::" << mContentFile << "::" << std::setw(8) << mIndex;
            return stream.str();

        case Index:
            stream << "sys::compat::index::" << std::setw(8) << mRecordType << "::" << std::setw(8) << mIndex;
            return stream.str();

        case ExteriorCell:
            stream << "sys::compat::exterior::" << std::dec << mX << "::" << mY;
            return stream.str();
...
CompatRefId ESMReader::getCompatRefIdValueData(uint32_t size)
{
    CompatRefId result;

    // Save format 23 and older stored RefIds as plain strings.
    if (getFormat() <= 23)
    {
        result.mType = CompatRefId::UnsizedString;
        result.mString = getString(static_cast<int>(size));
        return result;
    }
    ...
    getT(rawType);
    --size;
    result.mType = rawType <= CompatRefId::ExteriorCell
        ? static_cast<CompatRefId::Type>(rawType)
        : CompatRefId::Unknown;

    const auto skipRemainder = [this, &size]()
    {
        if (size)
        {
    ...
    switch (result.mType)
    {
        case CompatRefId::Empty:
            skipRemainder();
            break;

        case CompatRefId::SizedString:
        {
            if (size < sizeof(uint32_t))
                fail("Invalid sized RefId");

            uint32_t length = 0;
            getT(length);
            size -= sizeof(uint32_t);
            if (length > size)
                fail("Sized RefId exceeds subrecord size");

            result.mString = getString(static_cast<int>(length));
            size -= length;
            skipRemainder();
            break;
        }

        case CompatRefId::UnsizedString:
            result.mString = getString(static_cast<int>(size));
            size = 0;
            break;

        case CompatRefId::FormId:
            if (size < sizeof(uint32_t) + sizeof(int32_t))
                fail("Invalid FormId RefId");
            getT(result.mIndex);
            getT(result.mContentFile);
            size -= sizeof(uint32_t) + sizeof(int32_t);
            skipRemainder();
            break;

        case CompatRefId::Generated:
            if (size < sizeof(uint64_t))
                fail("Invalid generated RefId");
            getT(result.mGenerated);
            size -= sizeof(uint64_t);
            skipRemainder();
            break;

        case CompatRefId::Index:
            if (size < sizeof(uint32_t) * 2)
                fail("Invalid index RefId");
            getT(result.mRecordType);
            getT(result.mIndex);
            size -= sizeof(uint32_t) * 2;
            skipRemainder();
            break;

        case CompatRefId::ExteriorCell:
            if (size < sizeof(int32_t) * 2)
                fail("Invalid exterior-cell RefId");
            getT(result.mX);
            getT(result.mY);
...
CompatRefId ESMReader::getCompatRefIdValue()
{
    getSubHeader();
    return getCompatRefIdValueData(mCtx.leftSub);
}

CompatRefId ESMReader::getHNCompatRefIdValue(const char* name)
{
    getSubNameIs(name);
    return getCompatRefIdValue();
}

CompatRefId ESMReader::getHNOCompatRefIdValue(const char* name)
{
    if (isNextSub(name))
        return getCompatRefIdValue();
    return CompatRefId();
...
std::string ESMReader::getCompatRefIdData(uint32_t size)
{
    CompatRefId value = getCompatRefIdValueData(size);
    return value.toLegacyString();
}

std::string ESMReader::getCompatRefId()
{
    CompatRefId value = getCompatRefIdValue();
    return value.toLegacyString();
}

std::string ESMReader::getHNCompatRefId(const char* name)
{
    CompatRefId value = getHNCompatRefIdValue(name);
    return value.toLegacyString();
}

std::string ESMReader::getHNOCompatRefId(const char* name)
{
    CompatRefId value = getHNOCompatRefIdValue(name);
    return value.toLegacyString();
}
