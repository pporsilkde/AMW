...
namespace ESM
{
    ...
    void Dialogue::readInfo(ESMReader &esm, bool merge)
    {
        ...
        // Broken or cross-plugin PREV/NEXT chains should not make the response
        // disappear. Keep the INFO as a final fallback entry; later filtering
        // still decides whether an NPC can actually use it.
        mLookup[info.mId] = std::make_pair(mInfo.insert(mInfo.end(), info), isDeleted);
    }
