...
struct CompatRefId
{
  ...
  uint64_t mGenerated = 0;
  int32_t mX = 0;
  int32_t mY = 0;

  bool isString() const { return mType == SizedString || mType == UnsizedString; }

  /// Convert every supported RefId variant to a stable string key that can be
  /// stored by the legacy ArenaMW 0.47 ESM stores. String ids are preserved;
  /// generated/form/index ids use a reserved sys::compat namespace.
...
public:
  ...
  */
  ...
  /// Read a RefId from either the legacy string representation or the typed
  /// representation introduced by newer OpenMW save formats.
  CompatRefId getCompatRefIdValue();
  CompatRefId getHNCompatRefIdValue(const char* name);
  CompatRefId getHNOCompatRefIdValue(const char* name);
  CompatRefId getCompatRefIdValueData(uint32_t size);

  /// Convenience wrappers for legacy ArenaMW stores. Newer non-string RefIds
  /// are converted to stable reserved string keys so dynamic records and their
  /// references continue to match.
  std::string getCompatRefId();
  std::string getHNCompatRefId(const char* name);
  std::string getHNOCompatRefId(const char* name);
  std::string getCompatRefIdData(uint32_t size);
