...
bool Launcher::DataFilesPage::isGroundcoverCandidate(const QString& fileName)
{
    ...
    return lowered.contains(QLatin1String("groundcover"))
        || lowered.contains(QLatin1String("grass"));
}
...
bool Launcher::DataFilesPage::loadSettings()
{
    ...
    // the profile containing Morrowind.esm instead of leaving the empty
    // built-in Default profile selected.
...
void Launcher::DataFilesPage::populateFileViews(const QString& contentModelName)
{
    ...
    // Keep groundcover enabled by default until the profile contains an explicit
    // groundcover selection. This matches the shipped ArenaMW defaults.
    const bool storedGroundcoverEnabled = mLauncherSettings.isGroundcoverEnabled(contentModelName);
    const bool hasGroundcoverSelection
        = !mLauncherSettings.getGroundcoverFiles(contentModelName).isEmpty()
        || !mGameSettings.getGroundcoverList().isEmpty();
    ui.groundcoverCheckBox->setChecked(storedGroundcoverEnabled || !hasGroundcoverSelection);
}
...
void Launcher::DataFilesPage::saveSettings(const QString &profile)
{
   ...
   ContentSelectorModel::ContentFileList items = mSelector->selectedFiles();
    ...
    mLauncherSettings.setContentList(profileName, allFileNames,
        groundcoverFileNames, groundcoverEnabled);
...
void Launcher::DataFilesPage::on_cloneProfileAction_triggered()
{
    ...
    mLauncherSettings.setContentList(profile, sourceFiles,
        sourceGroundcoverFiles, sourceGroundcoverEnabled);
    mLauncherSettings.setCurrentContentListName(profile);
    addProfile(profile, true);
}
