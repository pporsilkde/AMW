...
void MWState::Character::addSlot (const boost::filesystem::path& path, const std::string& game)
{
    if (!boost::filesystem::is_regular_file(path)
        || Misc::StringUtils::lowerCase(path.extension().string()) != ".omwsave")
        return;
    ...
    // SAVE is normally the first record, but scan forward so future formats
    // may prepend metadata records without making the slot disappear from the
    // load menu.
    ...
    // Newer save formats may reorder the dependency list or omit the legacy
    // DEPE entries altogether. Only reject the slot when a dependency list is
    // present and none of its entries matches the active game file.
    if (!slot.mProfile.mContentFiles.empty())
    {
        const std::string lowerGame = Misc::StringUtils::lowerCase(game);
        bool matchesGame = false;
        for (std::vector<std::string>::const_iterator it = slot.mProfile.mContentFiles.begin();
             it != slot.mProfile.mContentFiles.end(); ++it)
        {
            if (Misc::StringUtils::lowerCase(*it) == lowerGame)
            {
                matchesGame = true;
                break;
            }
...
: mPath (saves)
{
    ...
    else
    {
            ...
            iter!=boost::filesystem::directory_iterator(); ++iter)
        {
            ...
            {
                Log(Debug::Warning) << "Warning: Could not read save slot " << slotPath.string()
                                    << ": " << e.what();
            }
            catch (...)
            {
