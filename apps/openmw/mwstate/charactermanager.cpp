...
: mPath (saves), mCurrent (nullptr), mGame (game)
{
    ...
    {
        // Normal OpenMW layout keeps saves in per-character directories.
        // Also accept loose .omwsave files copied directly into the saves
        // root, which is common when importing a save from another build.
        Character looseSaves(mPath, mGame);
        if (looseSaves.begin() != looseSaves.end())
            mCharacters.push_back(looseSaves);
