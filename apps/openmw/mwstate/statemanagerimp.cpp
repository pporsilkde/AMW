...
{
    // A live title-menu scene deliberately keeps State_NoGame while a real
    // world cell is loaded. It must be cleared before starting/loading a game.
    if (mState!=State_NoGame || force
        || MWBase::Environment::get().getWorld()->isMainMenuSceneActive())
    {
...
void MWState::StateManager::loadGame (const Character *character, const std::string& filepath)
{
    try
    {
        ...
        const int saveFormat = reader.getFormat();
        const bool forwardCompatibleLoad = saveFormat > ESM::SavedGame::sCurrentFormat;
        if (forwardCompatibleLoad)
        {
            Log(Debug::Warning) << "Warning: Save format " << saveFormat
                                << " is being converted to ArenaMW format "
                                << ESM::SavedGame::sCurrentFormat << ".";
        }
        ...
        while (reader.hasMoreRecs())
        {
            ...
            reader.getRecHeader();
            const ESM::ESM_Context recordContext = reader.getContext();
            ...
            try
            {
                switch (n.intval)
                {
                    ...
                    default:
                        ...
                        // Ignore records that this ArenaMW branch does not use.
                        reader.skipRecord();
                }

                // A known record may gain additional subrecords in a newer
                // format. The old parser has already consumed everything it
                // understands, so discard only the unconsumed tail.
                if (forwardCompatibleLoad && reader.hasMoreSubs())
                {
                    reader.skipRecord();
                }
            }
            catch (const std::exception&)
            {
                ...
                // Restore the exact start of the record before skipping it.
                // Individual loaders may have consumed only part of a newer
                // record before noticing an incompatible field.
                reader.restoreContext(recordContext);
                reader.skipRecord();
            }
            catch (...)
            {
...
bool MWState::StateManager::verifyProfile(const ESM::SavedGame& profile) const
{
    ...
    {
        const std::string lowerContentFile = Misc::StringUtils::lowerCase(contentFile);

        // New OpenMW versions list builtin Lua packages as save dependencies.
        // This old non-Lua branch cannot use them, so they are intentionally
        // ignored instead of blocking the load.
        if (lowerContentFile.size() >= 11
            && lowerContentFile.compare(lowerContentFile.size() - 11, 11, ".omwscripts") == 0)
        {
            continue;
        }

        bool found = false;
        for (const std::string& selected : selectedContentFiles)
        {
            if (Misc::StringUtils::lowerCase(selected) == lowerContentFile)
            {
                found = true;
                break;
            }
