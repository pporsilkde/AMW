...
{
    // Dynamic NPC poses must never drive the torso group. In OpenMW 0.47 the
    // neck and head live under the torso blend group, so upper-body pose clips
    // can otherwise inject authored head pitch and leave NPCs staring up/down.
    // Restrict ambient formal poses to the arms and let the normal look-at /
    // base idle controllers own the spine, neck and head.
...
namespace MWMechanics
{
    ...
    void Actors::updateDynamicIdleActor(const MWWorld::Ptr& ptr, Actor& actorState, float duration)
    {
        ...
        if (hardBlocked)
        {
            if (dialogueTarget && !state.mAnimation.empty())
            {
                ...
                // Ambient poses and dialogue poses intentionally reuse the same safe
                // group names (ArmsAtBack, ArmsFolded, ...). If dialogue has already
                // taken ownership of exactly that group, disabling the old ambient
                // state here would also kill the freshly-started dialogue animation.
                // Drop only the ambient bookkeeping in that case.
                if (dialogueWindow
        ...
        static const DynamicIdleAnimation sAnimations[] = {
            // Safe formal arm poses only. These are deliberately static/closed
            // stances; no preaching, scanning, copied idle or command clips are
            // injected automatically because those packs contain authored torso/
            // neck motion that can produce the forward/back head-tilt glitch.
            { "armsatback", 0.66f, 8, 12 },
            { "armsfolded", 0.66f, 8, 10 },
            { "armsakimbo", 0.66f, 8, 9 },
            { "handhippose", 0.60f, 8, 8 },

            // Guards strongly favour disciplined hands-behind-back / formal poses.
            { "armsatback", 0.64f, 8, 18, false, true, false },
            { "armsfolded", 0.64f, 8, 10, false, true, false },
            { "armsakimbo", 0.64f, 8, 8, false, true, false },

            // Religious and official/formal NPCs use the same safe pose family.
            // Context is retained only for weighting; no prayer/preach animations
            // are auto-played anymore.
            { "armsfolded", 0.64f, 8, 12, false, false, true },
            { "armsatback", 0.64f, 8, 10, false, false, true },
            { "handhippose", 0.60f, 8, 7, false, false, true },
            { "armsatback", 0.64f, 8, 14, false, false, false, true },
            { "armsfolded", 0.64f, 8, 12, false, false, false, true },
            { "armsakimbo", 0.64f, 8, 8, false, false, false, true },
        };

        std::vector<const DynamicIdleAnimation*> available;
        for (const DynamicIdleAnimation& candidate : sAnimations)
        {
            // xbase_animkna does not provide ArmPoses (armsatback/armsfolded).
            // Beast NPCs therefore use only their native KNA formal poses here;
            // otherwise hasAnimation() can be satisfied by the generic human
            // compatibility source and select a clip authored for the wrong rig.
    ...
    void Actors::engageCombat (const MWWorld::Ptr& actor1, const MWWorld::Ptr& actor2, std::map<const MWWorld::Ptr, const std::set<MWWorld::Ptr> >& cachedAllies, bool againstPlayer)
    {
        ...
        // but do not automatically protect the player from hostile NPCs.
