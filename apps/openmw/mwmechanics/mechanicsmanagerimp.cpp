...
namespace MWMechanics
{
    ...
    int MechanicsManager::getDerivedDisposition(const MWWorld::Ptr& ptr, bool addTemporaryDispositionChange, bool useBaseStats)
    {
        ...
        // Training prices must not be alterable through temporary magic. Keep Charm for normal
        // dialogue/barter, but ignore it when a caller explicitly requests base-stat pricing.
        if (!useBaseStats)
    ...
    int MechanicsManager::getBarterOffer(const MWWorld::Ptr& ptr, int basePrice, bool buying, bool useBaseStats)
    {
        ...
        // Normal commerce keeps vanilla modified-stat/fatigue behaviour. Training requests
        // useBaseStats=true so temporary Drain/Fortify effects and current stamina cannot
        // manipulate the price (including draining the trainer to force a near-1-gold quote).
        ...
        // Base-stat quotes intentionally use the *fully rested* fatigue term. This preserves
        // the normal rested-price balance while making the quote independent of current
        // stamina and temporary Fortify/Drain Fatigue effects.
        ...
        float pcTerm = (clampedDisposition - 50 + a + b + c) * pcFatigueTerm;
