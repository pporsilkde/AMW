...
namespace MWGui
{
    ...
    void LoadingScreen::setLabel(const std::string &label, bool important)
    {
        ...
        // Some localized content sets omit loading GMSTs. Resolve the simple
        // #{name} form safely so one absent string cannot leave the loading
        // window blank or abort GUI initialization while its bar is visible.
        if (label.size() > 3 && label.compare(0, 2, "#{") == 0 && label.back() == '}')
        {
            const std::string id = label.substr(2, label.size() - 3);
            mLoadingText->setCaption(
