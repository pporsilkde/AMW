...
namespace MWInput
{
    ...
    void ActionManager::update(float dt, bool triedToMove)
    {
        ...
        // ArenaMW placement mode: normal activation is deferred only for a
        ...
        if (mActivateHoldPending)
        {
            ...
            else if (stillHeld)
            {
                ...
                {
                    // The original reference must still be under the crosshair so
                    // the hold distance cannot accidentally come from another target.
                    ...
                    {
                        // The hold gesture is complete. Releasing Activate now keeps
                        // the object grabbed until Activate is pressed again.
                        mActivateHoldObject = MWWorld::Ptr();
                        mActivateHoldTime = 0.f;
                        mActivateHoldPending = false;
                    }
        ...
        if (MWBase::Environment::get().getInputManager()->getControlSwitch("playercontrols"))
        {
            ...
            if (physicsGrabMoveMode != 0)
            {
                // In an axis-placement mode the walking channels belong to the
                // held object, not the player.
    ...
    void ActionManager::executeAction(int action)
    {
        ...
        case A_Activate:
            ...
            if (!windowManager->isGuiMode() && inputManager->getControlSwitch("playercontrols"))
            {
                ...
                // A second Activate press ends placement. Physics ON releases a
                // live prop; Physics OFF freezes the exact arranged transform.
                if (world->isPhysicsGrabActive())
                {
                    world->finishPhysicsGrab();
                    mActivateHoldObject = MWWorld::Ptr();
        ...
        case A_MoveForward:
        case A_MoveBackward:
            // Axis-placement modes own movement actions exclusively. Walk mode
            // keeps the normal QuickLoot/list-navigation behaviour.
            if (MWBase::Environment::get().getWorld()->isPhysicsGrabActive()
