            ...
            case ESM::REC_DYNA:
                ...
                if (reader.getFormat() > 15)
                {
                    ...
                    if (reader.getSubSize() == 8)
                    {
                        uint64_t count = 0;
                        reader.getT(count);
                        mDynamicCount = static_cast<int>(std::min<uint64_t>(count, std::numeric_limits<int>::max()));
                    }
                    else if (reader.getSubSize() == 4)
                        reader.getT(mDynamicCount);
                    else
                        reader.fail("Unsupported DYNA counter size");
                }
                else
