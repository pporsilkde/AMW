    ...
    void Scene::loadCell (CellStore *cell, Loading::Listener* loadingListener, bool respawn, bool test)
    {
        ...
        if(result.second)
        {
            ...
            if (!test)
            {
                ...
                {
                    // configureAmbient sets the shader-side interior flag for
                    // true interiors. Reset it explicitly when returning to an
                    // exterior or quasi-exterior cell, otherwise the object and
                    // terrain shaders keep suppressing underwater caustics.
