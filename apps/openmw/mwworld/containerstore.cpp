...
void MWWorld::ContainerStore::readState (const ESM::InventoryState& inventory)
{
    ...
    for (const ESM::ObjectState& state : inventory.mItems)
    {
        ...
        switch (type)
        {
            ...
            case 0:
                // Expected when converting a save whose original mod list is
                // not fully available. The missing item is discarded silently;
                // the state manager already emitted the single conversion notice.
                break;
            default:
