...
namespace MWWorld
{
    ...
    void CellStore::readReferences (ESM::ESMReader& reader, const std::map<int, int>& contentFileMap, GetCellStoreCallback* callback)
    {
        ...
        while (reader.isNextSub ("OBJE"))
        {
            ...
            if (type == 0)
            {
                // Missing records are expected when importing a save made with
                // another mod list. Keep the log readable and skip only the
                // current object, never the remainder of the whole CSTA.
                if (reader.getFormat() <= 15)
                    Log(Debug::Warning) << "Dropping reference to '" << cref.mRefID << "' (object no longer exists)";

                while (reader.hasMoreSubs()
                    && !reader.peekNextSub("OBJE")
            ...
            catch (const std::exception&)
            {
                ...
                // A newer object can contain fields unknown to this branch.
                // Skip to the next independent object or moved-reference tag,
                // but stay inside the current CSTA record. The previous
                // skipHSubUntil("OBJE") could run past the record boundary and
                // produce thousands of bogus "SPAC got RESP" errors.
                while (reader.hasMoreSubs()
                    && !reader.peekNextSub("OBJE")
                    && !reader.peekNextSub("MVRF"))
                {
