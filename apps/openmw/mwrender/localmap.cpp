...
void LocalMap::requestInteriorMap(const MWWorld::CellStore* cell)
{
    ...
    if (cell->getFog())
    {
        ...
        if (std::abs(mAngle - fog->mNorthMarkerAngle) < osg::DegreesToRadians(5.f))
        {
            ...
            if (std::isfinite(fog->mCenterX) && std::isfinite(fog->mCenterY))
                mCenter = osg::Vec2f(fog->mCenterX, fog->mCenterY);
        }
    }
    ...
    for (int x = 0; x < segments.first; ++x)
    {
        for (int y = 0; y < segments.second; ++y)
        {
            ...
            if (!segment.mFogOfWarImage)
            {
                ...
                {
                    const auto match = std::find_if(fog->mFogTextures.begin(), fog->mFogTextures.end(),
                        [x, y, xOffset, yOffset](const ESM::FogTexture& texture)
                        {
...
void LocalMap::MapSegment::loadFogOfWar(const ESM::FogTexture &esm)
{
    ...
    if (!mFogOfWarImage)
    {
        Log(Debug::Error) << "Error: Fog image reader returned no image";
        initFogOfWar();
        return;
    }
