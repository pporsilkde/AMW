...
namespace
{
    ...
    void unequipRespawnDamageItems(const MWWorld::Ptr& player)
    {
        ...
        {
            MWWorld::ConstContainerStoreIterator item = inventory.getSlot(slot);
            if (item == inventory.end())
                continue;

            const std::string enchantmentId = item->getClass().getEnchantment(*item);
            if (enchantmentId.empty())
                continue;

            const ESM::Enchantment* enchantment
                = world->getStore().get<ESM::Enchantment>().find(enchantmentId);
            if (enchantment->mData.mType != ESM::Enchantment::ConstantEffect)
                continue;

            const bool damaging = std::any_of(enchantment->mEffects.mList.begin(),
                enchantment->mEffects.mList.end(), [](const ESM::ENAMstruct& effect)
                {
                    return std::find(damageEffects.begin(), damageEffects.end(),
                        effect.mEffectID) != damageEffects.end();
                });
            if (damaging)
                inventory.unequipSlot(slot, player);
        }
    }

    void restoreRespawnResourcesToTenPercent(MWMechanics::CreatureStats& stats)
    {
        MWMechanics::DynamicStat<float> health = stats.getHealth();
        health.setCurrent(std::max(1.f, health.getModified() * 0.10f));
        stats.setHealth(health);

        MWMechanics::DynamicStat<float> magicka = stats.getMagicka();
        magicka.setCurrent(std::max(0.f, magicka.getModified() * 0.10f));
        stats.setMagicka(magicka);

        MWMechanics::DynamicStat<float> fatigue = stats.getFatigue();
        fatigue.setCurrent(std::max(1.f, fatigue.getModified() * 0.10f));
        stats.setFatigue(fatigue);
    }

    void applyAzuraSkillPenalty(const MWWorld::Ptr& player)
    {
        MWMechanics::CreatureStats& creatureStats = player.getClass().getCreatureStats(player);
        const int level = std::max(1, creatureStats.getLevel());

        // Levels 1-5 are protected. Afterwards one different random skill loses
        // one base point for every complete five-level band: 6-10 => 1,
        // 11-15 => 2, and so on.
        const int penaltyCount = (level - 1) / 5;
        if (penaltyCount <= 0 || !player.getClass().isNpc())
            return;

        MWMechanics::NpcStats& npcStats = player.getClass().getNpcStats(player);
        std::vector<int> candidates;
        candidates.reserve(ESM::Skill::Length);
        for (int skill = 0; skill < ESM::Skill::Length; ++skill)
        {
            if (npcStats.getSkill(skill).getBase() > 1.f)
                candidates.push_back(skill);
        }

        const int count = std::min(penaltyCount, static_cast<int>(candidates.size()));
        for (int i = 0; i < count; ++i)
        {
            const int selected = Misc::Rng::rollDice(static_cast<int>(candidates.size()));
            const int skill = candidates[selected];
            candidates[selected] = candidates.back();
            candidates.pop_back();

            MWMechanics::SkillValue& value = npcStats.getSkill(skill);
            value.setBase(std::max(1.f, value.getBase() - 1.f));
        }
    }
    ...
    std::string trimDisplaySetting(const std::string& value)
    {
    ...
    bool hasDisplaySetting(const std::vector<std::string>& lines,
        const std::string& section, const std::string& key)
    {
        std::string currentSection;
        for (const std::string& rawLine : lines)
        {
            const std::string line = trimDisplaySetting(rawLine);
            ...
            if (equals != std::string::npos
                && trimDisplaySetting(line.substr(0, equals)) == key)
                return true;
        }
        return false;
    }

    void appendDisplaySetting(std::vector<std::string>& lines,
        const std::string& section, const std::string& key, const std::string& value)
    {
        if (hasDisplaySetting(lines, section, key))
            return;
        ...
        {
            const std::string line = trimDisplaySetting(lines[i]);
            ...
            if (line.substr(1, end - 1) == section)
            {
                sectionFound = true;
                insertPos = i + 1;
            }
        ...
        {
            if (!lines.empty() && !trimDisplaySetting(lines.back()).empty())
    ...
    void ensureNativeDisplaySettings(const boost::filesystem::path& settingsPath)
    {
        int width = 1920;
        int height = 1080;
        ...
        float factor = static_cast<float>(std::max(width, height)) / 1280.f;
        factor = std::max(0.5f, std::min(4.f, factor));
        scaling << std::fixed << std::setprecision(2) << factor;

        appendDisplaySetting(lines, "Video", "resolution x", std::to_string(width));
        appendDisplaySetting(lines, "Video", "resolution y", std::to_string(height));
        appendDisplaySetting(lines, "Video", "screen", "0");
        appendDisplaySetting(lines, "Video", "fullscreen", "true");
        appendDisplaySetting(lines, "GUI", "scaling factor", scaling.str());

        if (boost::filesystem::exists(settingsPath) && oldSize == lines.size())
            return;
...
bool OMW::Engine::frame(float frametime)
{
    try
    {
        ...
        // Only the Escape/main menu pauses the standalone simulation. Inventory,
        // dialogue, barter, containers, books, the console and modal message
        // boxes remain live, matching Arena's multiplayer-derived GUI flow.
        const bool paused
            = mEnvironment.getWindowManager()->containsMode(MWGui::GM_MainMenu);
        const bool liveMainMenuScene = mEnvironment.getWorld()->isMainMenuSceneActive();
        // The title menu still blocks normal gameplay and scripts, but its
        // background world is allowed to animate water/weather and run actor AI.
        const bool simulationPaused = paused && !liveMainMenuScene;

        // ArenaMP evaluates persistent poses after input but before mechanics.
        // CharacterController consumes and clears the movement vector during
        // its update, so doing this afterwards made an active pose permanently
        // mask player movement.
        if (!paused
            && mEnvironment.getStateManager()->getState() != MWBase::StateManager::State_NoGame)
        {
            ArenaMW::updateConsumingAnimations(frametime);
            ArenaMW::updatePersistentAnimation(
                mEnvironment.getWorld()->getPlayerPtr(), frametime);
        }
        ...
        // update mechanics
        {
            ...
            if (mEnvironment.getStateManager()->getState() != MWBase::StateManager::State_NoGame
                || liveMainMenuScene)
            {
                // In the live title scene this advances existing AI packages and
                // actor animation only. Script execution and game-time advancement
                // remain disabled by the State_NoGame guards above.
                mEnvironment.getMechanicsManager()->update(frametime, simulationPaused);
                if (!simulationPaused
                    && mEnvironment.getStateManager()->getState() != MWBase::StateManager::State_NoGame)
                    MWWorld::InteractionAnimation::update(frametime);
            }
            ...
            if (mEnvironment.getStateManager()->getState() == MWBase::StateManager::State_Running)
            {
                ...
                if (!paused && stats.isDead())
                {
                    ...
                    if (mRespawnTimer >= delay)
                    {
                        ...
                        // references obtained from the old player Ptr.
                        mEnvironment.getMechanicsManager()->resurrect(player);

                        MWWorld::Ptr revivedPlayer = mEnvironment.getWorld()->getPlayerPtr();
                        MWMechanics::CreatureStats& revivedStats
                            = revivedPlayer.getClass().getCreatureStats(revivedPlayer);
                        ...
                        // Teleport last. Do not use revivedPlayer/revivedStats afterwards,
                        // because changing cells can invalidate references into the old cell.
        ...
        // update physics
        {
            ...
            if (mEnvironment.getStateManager()->getState() != MWBase::StateManager::State_NoGame
                || mEnvironment.getWorld()->isMainMenuSceneActive())
            {
                mEnvironment.getWorld()->updatePhysics(frametime, simulationPaused, frameStart, frameNumber, *stats);
            }
        }
        ...
        // update world
        {
            ...
            if (mEnvironment.getStateManager()->getState() != MWBase::StateManager::State_NoGame
                || mEnvironment.getWorld()->isMainMenuSceneActive())
            {
                mEnvironment.getWorld()->update(frametime, simulationPaused);
            }
        }
...
std::string OMW::Engine::loadSettings (Settings::Manager & settings)
{
    ...
    const boost::filesystem::path settingsFile = mCfgMgr.getUserConfigPath() / "settings.cfg";
    ensureNativeDisplaySettings(settingsFile);
    const std::string settingspath = settingsFile.string();
...
void OMW::Engine::prepareEngine (Settings::Manager & settings)
{
    ...
    // ArenaMW uses a live in-engine title scene instead of startup movies.
...
void OMW::Engine::go()
{
    ...
    {
        // Create the live GRID -3,-2 scene before opening the title menu.
        // This is the original working startup order: MainMenu only updates
        // the scene and never clears/restarts it after the GUI is visible.
        mEnvironment.getWorld()->startMainMenuScene("");
    ...
    while (!mViewer->done() && !mEnvironment.getStateManager()->hasQuitRequest())
    {
        ...
        else
        {
            ...
            const bool paused
                = mEnvironment.getWindowManager()->containsMode(MWGui::GM_MainMenu);
            // Water and terrain shaders use osg_SimulationTime directly. Keep
            // that clock advancing only for the live title scene; a normal
            // paused game still freezes exactly as before.
            if (!paused || mEnvironment.getWorld()->isMainMenuSceneActive())
                simulationTime += dt;
        }
