    ...
    void DialogueManager::keywordSelected (const std::string& keyword, ResponseCallback* callback)
    {
        if(!mIsInChoice)
        {
            ...
            if (dialogue && dialogue->mType == ESM::Dialogue::Topic)
            {
                ...
                // While the dialogue window is open, play one ambient NPC phrase
                // after every seven topic selections instead of using a random chance.
                constexpr unsigned int topicSelectionsPerReaction = 7;
                ++mTopicSelectionCount;
                if (mTopicSelectionCount >= topicSelectionsPerReaction)
                {
                    mTopicSelectionCount = 0;
                    if (!voicePlayed
