...
printf "Qt ${QT_VER}... "
{
	...
	for CONFIGURATION in ${CONFIGURATIONS[@]}; do
		...
		add_runtime_dlls $CONFIGURATION \
			"$(pwd)/bin/Qt${QT_VER:0:1}"{Core,Gui,Widgets}${DLLSUFFIX}.dll
...
# ArenaMW standalone package: keep the game, launcher, setup wizard and INI importer while
# excluding editor/conversion/test utilities that are not shipped to players.
# This shortens CI builds and removes the only QtNetwork consumer (OpenMW-CS)
# without changing the launcher's normal OpenMW configuration workflow.
